...
def generateTwoDArray(list):
  ...
  while i<len(list):
    ...
    listOfWordsTraining.append([tag,wordWithOutSpecialChars])
...
def NLBUnigram(type, train_df_tagged, test_df_tagged, typeOfNGram):
  # vectorize the training data set
  cv_unigram=None;
  stringVal = ""
  if typeOfNGram=="Unigram":
    cv_unigram = CountVectorizer(ngram_range=(1, 1))
    stringVal = "score_val_unigrams "
  elif typeOfNGram =="Bigram":
    cv_unigram = CountVectorizer(ngram_range=(1, 1))
    stringVal = "score_val_bigrams "
  else:
    cv_unigram = CountVectorizer(ngram_range=(1, 2))
    stringVal = "score_val_unigrams_bigrams "
  train_unigram_mapping = cv_unigram.fit_transform(column(train_df_tagged,1))
  # vectorize the testing data set
  test_unigram_mapping = cv_unigram.transform(column(test_df_tagged,1))
  # train & predict MultinomialNB on unigram
  NLB = MultinomialNB()
  model_unigrams = NLB.fit(train_unigram_mapping, column(train_df_tagged,0))
  predictions_unigrams = model_unigrams.predict(test_unigram_mapping)
  print(predictions_unigrams)
  score_val_unigrams = metrics.accuracy_score(column(test_df_tagged,0), predictions_unigrams)
...
NLBUnigram("withstopwords",train_df_tagged,test_df_tagged,"Unigram")
#bigrams with stopwords
NLBUnigram("withstopwords",train_df_tagged,test_df_tagged,"Bigram")
